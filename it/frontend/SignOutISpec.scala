  ...
  "Signing out from the welcome page" should {
    "redirect the user to the exit questionnaire and clear the session" when {
      "hitting /register-for-paye/sign-out" in {
        ...
        val welcomeResponse = await(buildClient("/").withHeaders(HeaderNames.COOKIE -> getSessionCookie()).get())
        val welcomeCookies = welcomeResponse.cookies
        ...
        assert(welcomeCookies != signOutResponse.cookies)
      }
    }
  }
}
