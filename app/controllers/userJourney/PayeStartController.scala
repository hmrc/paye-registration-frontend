...
trait PayeStartCtrl extends FrontendController with Actions with I18nSupport {
      ...
      implicit request =>
        hasOrgAffinity {
          checkAndStoreCurrentProfile {
            profile =>
              checkAndStoreCompanyDetails(profile.companyTaxRegistration.transactionId) {
  ...
  private def checkAndStoreCompanyDetails(txID: String)(f: => Future[Result])(implicit hc: HeaderCarrier, request: Request[AnyContent]): Future[Result] = {
    coHoAPIService.fetchAndStoreCoHoCompanyDetails(txID) flatMap {
