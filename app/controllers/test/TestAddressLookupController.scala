...
trait TestAddressLookupController extends PayeBaseController {
  ...
  def noLookupPPOBAddress: Action[AnyContent] = isAuthorisedWithProfile { implicit request =>
    profile =>
      val address = Address(
  ...
  def noLookupCorrespondenceAddress: Action[AnyContent] = isAuthorisedWithProfile { implicit request =>
    profile =>
      val address = Address(
