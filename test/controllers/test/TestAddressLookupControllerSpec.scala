...
class TestAddressLookupControllerSpec extends PAYERegSpec {
  ...
  "calling the noLookup action for PAYE Correspondence Address" should {
    ...
    "return 500 when the mocked address can't be submitted" in new Setup {
      when(mockPAYEContactService.saveCorrespondenceAddress(ArgumentMatchers.anyString(), ArgumentMatchers.any())(ArgumentMatchers.any())).thenReturn(Future.successful(DownstreamOutcome.Failure))
    ...
    "return 303 when the mocked address is successfully submitted" in new Setup {
      when(mockPAYEContactService.saveCorrespondenceAddress(ArgumentMatchers.anyString(), ArgumentMatchers.any())(ArgumentMatchers.any())).thenReturn(Future.successful(DownstreamOutcome.Success))
